...
/**
 * Šiame pavyzdyje parodoma kaip nusiųsti daugiau nei vieną skelbimą
 * į rinka.lt portalą.
...
// ANTRO skelbimo sukūrimas
    $Entry2 = $ExportDocument->createNewInsertEntry($Category);

    // Jūsų sistemoje sugeneruotas unikalus ID
    $Entry2->setLocalId(123);

    // Kontaktų pridėjimas
    $Entry2->addContact('miestas', array('lietuva', 'vilnius'));
    $Entry2->addContact(array(
        'el_pastas' => 'username@domain.tld',
        'telefonas' => '+37060000000'
    ));

    // Paveikslėlių pridėjimas
    $Entry2->addImage(array(
        'http://domain.tld/1.jpg',
        'http://domain.tld/2.jpg',
    ));

    // Galiojimo datos
    $Entry2->setPublishDate(date('Y-m-d'));
    $Entry2->setPublishUntil(date('Y-m-d', strtotime('+3 MONTH')));

    // Nuoroda į skelbimą jūsų puslapyje
    $Entry2->setSourceUrl('http://your_domain.tld/advertisiment/1234');

    // Laukelių pridėjimas

        // Markė
        $Entry2->setFieldValue('marke', array('ac', 'aro', 'dodge', 'ferrari', 'honda', 'isuzu', 'jeep', 'lincoln', 'mercedes_benz', 'moskvich', 'nissan', 'oldsmobile', 'peugeot', 'plymouth', 'renault', 'santana', 'saturn', 'smart', 'ssang_yong'));

        // Modelis
        $Entry2->setFieldValue('modelis', 'tekstinis laukas');

        // Pagaminimo data (nuo-iki)
        $Entry2->setFieldValue('pagaminimo_data', 123, 123);

        // Darbinis tūris, cm³ (nuo-iki)
        $Entry2->setFieldValue('darbinis_turis_cm', 123, 123);

        // Galia, AG/kW
        $Entry2->setFieldValue('galia_ag_kw', 123, 123, 'kw');

        // Kuro tipas
        $Entry2->setFieldValue('kuro_tipas', 'dujos');

        // Pavarų dėžė
        $Entry2->setFieldValue('pavaru_deze', '');

        // Kėbulas
        $Entry2->setFieldValue('kebulas', array('hecbekas', 'sedanas'));

        // Durų skaičius
        $Entry2->setFieldValue('duru_skaicius', '');

        // Spalva
        $Entry2->setFieldValue('spalva', '');

        // Kiti ypatumai
        $Entry2->setFieldValue('kiti_ypatumai', array('naudotas', 'dauztas', 'remontuotinas', 'su_defektais', 'pavaru_dezes_defektas', 'sankabos_defektas', 'variklio_defektas', 'vaziuokles_defektas'));

        // Papildoma informacija
        $Entry2->setFieldValue('papildoma_informacija', 'tekstinis laukas');

        // Kiek mokės
        $Entry2->setFieldValue('kiek_mokes', 123, 'pln');
